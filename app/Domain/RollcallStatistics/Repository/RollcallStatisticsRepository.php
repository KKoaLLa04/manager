...
class RollcallStatisticsRepository {
    ...
    //             // Kiểm tra nếu người dùng nhập ngày cụ thể (d/m/yyyy)
    //             try {
                  
    //                 $dateParsed = \Carbon\Carbon::createFromFormat('Y-m-d', $date);
    //                 $startDate = $dateParsed->startOfMonth();  // Lấy ngày đầu tháng của tháng đã nhập
    //                 $endDate = $dateParsed->endOfMonth();      // Lấy ngày cuối tháng của tháng đã nhập
    //             } catch (\Exception $e) {
    //                 // Nếu người dùng chỉ nhập tháng và năm (mm/yyyy), sẽ lấy toàn bộ tháng đó
    //                 try {
    //                     $dateParsed = \Carbon\Carbon::createFromFormat('m/Y', $date);
    //                     $startDate = $dateParsed->startOfMonth();  // Lấy ngày đầu tháng
    //                     $endDate = $dateParsed->endOfMonth();      // Lấy ngày cuối tháng
    //                 } catch (\Exception $e) {
    //                     return response()->json(['message' => 'Ngày tháng không hợp lệ'], 400);  // Lỗi nếu không thể phân tích ngày tháng
    //                 }
    //             }
    //         } else {
    //             // Nếu không có ngày, lấy toàn bộ tháng hiện tại
    ...
    //         // Lấy tất cả các ngày trong khoảng thời gian từ ngày đầu tiên điểm danh đến ngày hiện tại
    //         $startDate = RollCallHistory::where('student_id', $student->id)
    //             ->where('class_id', $studentClassHistory->class_id)
    //             ->orderBy('date', 'asc')
    //             ->value('date'); // Lấy ngày đầu tiên điểm danh
    ...
    //       // Lấy chi tiết từng ngày với trạng thái (có trạng thái cho ngày điểm danh, còn lại là ngày nghỉ)
    //         $attendanceDetails = $dateRange->map(function ($date) use ($student, $studentClassHistory) {
    //             // Kiểm tra nếu ngày có điểm danh
    //             $attendance = RollCallHistory::where('student_id', $student->id)
    //                 ->where('class_id', $studentClassHistory->class_id)
    //                 ->whereDate('date', '=', \Carbon\Carbon::createFromFormat('d/m/Y', $date)->format('Y-m-d')) // Đảm bảo chuyển định dạng về Y-m-d
    //                 ->first();

    //             // Nếu không có điểm danh trong ngày, trả về status = 5 (ngày lễ hoặc không điểm danh)
    //             if ($attendance) {
    //                 return [
    //                     'date' => $date,
    ...
    //                 return [
    //                     'date' => $date,
    //                     'status' => StatusStudentEnum::HOLIDAY->value, // Ngày không có điểm danh
    //                 ];
    //             }
    //         });
