...
class RollCallTeacherRespository
{
    public function getClass($pageIndex = 1, $pageSize = 10, $keyWord = null, $date = null)
    {
        ...
        // Lấy ID của giáo viên chủ nhiệm hiện tại
        $currentTeacherId = auth()->user()->id;
        ...
        $query = Classes::where('is_deleted', DeleteEnum::NOT_DELETE->value)
            ...
            // Lọc lớp có giáo viên chủ nhiệm là người đang truy cập
            ->whereHas('classSubjectTeacher', function ($query) use ($currentTeacherId) {
                // Chỉ lấy lớp mà giáo viên chủ nhiệm có quyền truy cập
                $query->where('access_type', StatusTeacherEnum::MAIN_TEACHER->value)
                    ->where('is_deleted', DeleteEnum::NOT_DELETE->value)
                    ->where('user_id', $currentTeacherId); // Lọc theo ID giáo viên chủ nhiệm
            })
            ->with([
                'grade',
        ...
        ->whereHas('classSubjectTeacher', function ($query) {
            // Lọc theo giáo viên chủ nhiệm của lớp
            $query->where('access_type', StatusTeacherEnum::MAIN_TEACHER->value)
                ->where('is_deleted', DeleteEnum::NOT_DELETE->value)
                ->where('user_id', auth()->user()->id); // Lọc theo ID giáo viên chủ nhiệm hiện tại
        })
        ->count();
...
{
    // Lọc lớp mà giáo viên chủ nhiệm hiện tại đang quản lý
    $classSubjectTeacher = ClassSubjectTeacher::where('class_id', $class_id)
        ->where('access_type', StatusTeacherEnum::MAIN_TEACHER->value)  // Chỉ lấy giáo viên chủ nhiệm
        ->where('is_deleted', DeleteEnum::NOT_DELETE->value)
        ->where('user_id', auth()->user()->id) // Lọc theo giáo viên chủ nhiệm hiện tại
        ->first();

    if (!$classSubjectTeacher) {
        // Nếu không tìm thấy lớp với giáo viên chủ nhiệm hiện tại, trả về thông báo lỗi
        return [
            'message' => 'Bạn không có quyền xem học sinh trong lớp này.',
            'data' => [],
        ];
    }

    // Truy vấn học sinh trong lớp mà giáo viên chủ nhiệm đang quản lý
    ...
    $totalStudent = $studentsQuery->count();

    // Lấy số học sinh đã điểm danh
    $toltalStudentAttendance = RollCall::where('class_id', $class_id)
        ->where('is_deleted', DeleteEnum::NOT_DELETE->value)
        ->distinct('student_id')
        ->count();
    ...
    return [
        ...
        'data' => $students->map(function ($classHistory) {
            ...
            $rollcall = $classHistory->class->rollCalls->first(); // Lấy bản ghi rollCall đầu tiên (nếu có)

            // Kiểm tra xem học sinh có điểm danh hay không, nếu không có thì trả về 'Không có ghi chú'
            $note = $rollcall ? $rollcall->note : 'Không có ghi chú';
